...
static bool
isHungarianNotationSupportedStyle(int StyleKindIndex) {

	if ((StyleKindIndex >= SK_EnumConstant) && (StyleKindIndex <= SK_ConstantParameter))
		return true;
...
getFileStyleFromOptions(const ClangTidyCheck::OptionsView &Options) {
  ...
  for (unsigned I = 0; I < SK_Count; ++I) {
    ...
    auto HPTOpt = 
        Options.getOptional<IdentifierNamingCheck::HungarianPrefixType>(StyleString, isHungarianNotationSupportedStyle(I));

    //if (!isHungarianNotationSupportedStyle(I)) {
    //    //llvm::Expected<llvm::Error> Val = llvm::make_error<UnsupportedOptionError>(("OOOOOO" + StyleString).str());
